...
# Cognito Setup Automation Script for Argus
...
POOL_NAME="${COGNITO_POOL_NAME:-argus-sre-agent-user-pool}"
DOMAIN_PREFIX="${COGNITO_DOMAIN_PREFIX:-argus-sre-agent-$(date +%s)}"
RESOURCE_SERVER_ID="${COGNITO_RESOURCE_SERVER_ID:-argus-sre-agent-api}"
CLIENT_NAME="${COGNITO_CLIENT_NAME:-argus-sre-agent-client}"
...
create_user_pool_domain() {
    ...
    if aws cognito-idp describe-user-pool-domain --domain "$DOMAIN_PREFIX" --region "$REGION" &> /dev/null; then
        ...
        DOMAIN_PREFIX="argus-sre-agent-$(date +%s)-$(shuf -i 100-999 -n 1)"
...
create_resource_server() {
    ...
    aws cognito-idp create-resource-server \
        ...
        --name "Argus API Resource Server" \
        --scopes '[
            {"ScopeName":"read","ScopeDescription":"Read access to Argus APIs"}, 
            {"ScopeName":"write","ScopeDescription":"Write access to Argus APIs"}
        ]'
...
generate_env_file() {
    ...
    cat > "$ENV_FILE" << EOF
# Cognito Configuration for Argus
...
# Main function
main() {
    echo "🚀 Starting Cognito Setup for Argus..."
    echo ""
