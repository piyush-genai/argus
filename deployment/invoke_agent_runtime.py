...
def main():
    parser = argparse.ArgumentParser(
        description="Invoke Argus Runtime via AgentCore"
    )
    ...
    if not runtime_arn:
        ...
        if arn_file.exists():
            ...
            if uri_file.exists():
                ...
                runtime_arn = f"arn:aws:bedrock-agentcore:{args.region}:{account_id}:runtime/argus-sre-agent"
                logging.info(
    ...
    if not session_id:
        ...
        session_id = f"argus-sre-agent-session-{timestamp}-invoke"
