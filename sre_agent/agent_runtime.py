...
# Simple FastAPI app
app = FastAPI(title="Argus Runtime", version="1.0.0")
...
async def initialize_agent():
    ...
    try:
        ...
        logger.info(
            f"Argus system initialized successfully with {len(tools)} tools"
        )
        ...
        if isinstance(e, (LLMAuthenticationError, LLMAccessError, LLMProviderError)):
            ...
            logger.error(f"Failed to initialize Argus system: {e}")
        raise
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Argus Runtime")
    parser.add_argument(
        "--provider",
    ...
    logger.info(f"Starting Argus Runtime with provider: {args.provider}")
    if debug_enabled:
