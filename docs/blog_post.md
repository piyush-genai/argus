...
![Argus Architecture with AgentCore Components](./images/argus-sre-agent-architecture.png)
...
**AgentCore Memory**: Transforms the Argus from a stateless system into an intelligent, learning assistant that personalizes investigations based on user preferences and historical context. The memory system provides three distinct strategies:
...
The Argus follows a structured deployment process from local development to production:

```
STEP 1: LOCAL DEVELOPMENT
┌─────────────────────────────────────────────────────────────────────┐
│  Develop Python Package (sre_agent/)                                │
│  └─> Test locally with CLI: uv run argus-sre-agent --prompt "..."         │
...
- [Use Case Setup Guide](https://github.com/Dev-Shubham/argus-argus-sre-agent/tree/main/02-use-cases/argus-argus-sre-agent#use-case-setup) - Backend deployment and development setup
- [Deployment Guide](https://github.com/Dev-Shubham/argus-argus-sre-agent/tree/main/02-use-cases/argus-argus-sre-agent/docs/deployment-guide.md) - Production containerization and AgentCore Runtime deployment
...
The gateway process begins by uploading your existing API specifications to Amazon S3. The [create_gateway.sh](https://github.com/Dev-Shubham/argus-argus-sre-agent/blob/feature/issue-143-deploy-argus-sre-agent-agentcore-runtime/02-use-cases/argus-argus-sre-agent/gateway/create_gateway.sh) script automatically handles uploading the four API specifications (Kubernetes, Logs, Metrics, and Runbooks) to your configured S3 bucket with proper metadata and content types. These specifications will be used to create API Endpoint Targets in the gateway.

#### Step 2: Create identity provider and gateway

Authentication is handled seamlessly through AgentCore Identity. The [main.py](https://github.com/Dev-Shubham/argus-argus-sre-agent/blob/feature/issue-143-deploy-argus-sre-agent-agentcore-runtime/02-use-cases/argus-argus-sre-agent/gateway/main.py) script creates both the credential provider and gateway:
...
Once deployed, the AgentCore Gateway provides a standardized `/mcp` endpoint secured with JWT tokens. Testing the deployment with [mcp_cmds.sh](https://github.com/Dev-Shubham/argus-argus-sre-agent/blob/feature/issue-143-deploy-argus-sre-agent-agentcore-runtime/02-use-cases/argus-argus-sre-agent/gateway/mcp_cmds.sh) reveals the power of this transformation:
...
- **[LangGraph](https://langchain-ai.github.io/langgraph/)**: The framework used in our Argus implementation  
...
While AgentCore Gateway provides seamless API access, AgentCore Memory transforms the Argus from a stateless system into an intelligent, learning assistant. The memory implementation demonstrates how a few lines of code can enable sophisticated personalization and cross-session knowledge retention.

#### Step 1: Initialize Memory Strategies

The Argus memory system is built on Amazon Bedrock AgentCore Memory's event-based model with automatic namespace routing. During initialization, the system creates three memory strategies with specific namespace patterns:
...
The system comes pre-configured with user personas that demonstrate personalized investigations. The [manage_memories.py](https://github.com/Dev-Shubham/argus-argus-sre-agent/blob/feature/issue-143-deploy-argus-sre-agent-agentcore-runtime/02-use-cases/argus-argus-sre-agent/scripts/manage_memories.py) script loads these personas:
...
The power of AgentCore Memory lies in its automatic namespace routing. When the Argus creates events, it only needs to provide the `actor_id`—Amazon Bedrock AgentCore Memory automatically determines which namespace(s) the event belongs to:
...
AgentCore Runtime requires ARM64 containers. Here's the complete [Dockerfile](https://github.com/Dev-Shubham/argus-argus-sre-agent/blob/feature/issue-143-deploy-argus-sre-agent-agentcore-runtime/02-use-cases/argus-argus-sre-agent/Dockerfile):
...
Deploying to AgentCore Runtime is surprisingly straightforward with the [deploy_agent_runtime.py](https://github.com/Dev-Shubham/argus-argus-sre-agent/blob/feature/issue-143-deploy-argus-sre-agent-agentcore-runtime/02-use-cases/argus-argus-sre-agent/deployment/deploy_agent_runtime.py) script:
...
Calling your deployed agent is just as simple with [invoke_agent_runtime.py](https://github.com/Dev-Shubham/argus-argus-sre-agent/blob/feature/issue-143-deploy-argus-sre-agent-agentcore-runtime/02-use-cases/argus-argus-sre-agent/deployment/invoke_agent_runtime.py):
...
The complete deployment process, including building containers and handling AWS permissions, is documented in the [Deployment Guide](https://github.com/Dev-Shubham/argus-argus-sre-agent/blob/feature/issue-143-deploy-argus-sre-agent-agentcore-runtime/02-use-cases/argus-argus-sre-agent/docs/deployment-guide.md).


## Real-world use cases

Let's explore how the Argus handles common incident response scenarios with a real investigation.
...
argus-sre-agent --prompt "API response times have degraded 3x in the last hour"
```
...
The Argus demonstrates how multi-agent systems can transform incident response from a manual, time-intensive process into an intelligent, collaborative investigation that provides SREs with the insights they need to resolve issues quickly and confidently.
...
* [Argus GitHub repository](https://github.com/Dev-Shubham/argus-argus-sre-agent)
