# Argus Deployment Guide for Amazon Bedrock AgentCore Runtime

This guide walks you through the complete deployment process for the Argus, from local testing to production deployment on Amazon Bedrock AgentCore Runtime.
...
The Argus uses environment variables for configuration. These are read from `.env` files in the appropriate directories:
...
uv run argus-sre-agent --prompt "list the pods in my infrastructure"

# Test with debug output enabled
uv run argus-sre-agent --prompt "list the pods in my infrastructure" --debug

# Test with specific provider
uv run argus-sre-agent --prompt "list the pods in my infrastructure" --provider bedrock --debug
```
...
uv run argus-sre-agent --prompt "list the pods in my infrastructure" --provider anthropic --debug
```
...
uv run argus-sre-agent --prompt "your query" --debug

# Or with environment variable
DEBUG=true uv run argus-sre-agent --prompt "your query"
```
...
uv run argus-sre-agent --provider bedrock --prompt "your query"
...
uv run argus-sre-agent --provider anthropic --prompt "your query"
...
3. **Debug Information**
   ...
   DEBUG=true uv run argus-sre-agent --prompt "test"
   ```
