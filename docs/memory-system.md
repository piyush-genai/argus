...
When the Argus initializes, it creates three memory strategies with specific namespace patterns:
...
The key insight is that **the Argus only needs to provide the actor_id** when calling `create_event()`. Amazon Bedrock AgentCore Memory automatically:
...
### Example Event Flow
```python
# Argus calls create_event with just actor_id and content
...
2. Use retrieve_memory("infrastructure", "[relevant service terms from query]", "argus-sre-agent", 10, null)  
...
The Argus automatically captures information during investigations through a sophisticated pattern recognition and structured data conversion process:
